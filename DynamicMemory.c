...
void Introduce_student(char student_name[], int id, struct Student** head_ref) {
    ...
    //Insert at beginning of linked list or between previous and current node
    if (NULL == previous) {
    ...
    //Initialize marks for all modules
    for (int i = 0; i < 5; i++) {
...
void SavetoFile(struct Student *head_ref) {
    ...
    if (pFile != NULL) {
		...
		printf("FILE OPENING ERROR FOR WRITE\n");
	}
}

//  [LINKED LIST]  7.0) Function to put the data read from file into the linked list
struct Student* ReadNextFromFile(struct Student* head_ref, FILE* pFile) {
    size_t returnValue;
    //Putting retrieved student into linked list if it's empty
    if (head_ref == NULL) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        //If the user chooses binary tree
            ...
            //Keep displaying the menu until the user decides to quit the program
            do{
                //Purge input if the user inputs an invalid character - works on my mac but somehow gives error on windows - comment out if needed
                fpurge(stdin);
                //Main menu
                printf("\nManage data for students: (Type an option and press ENTER)\n");
                printf("1) Introduce new student:\n");
